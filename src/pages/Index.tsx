...
const Index = () => {
  ...
  return (
    <div className="min-h-screen pb-28 bg-background">
      ...
      {/* Main Content */}
      <div className="w-full max-w-screen-md px-5 mx-auto pt-24 pb-8 animate-fade-in">
        {activeMainTab === 'track' ? (
          <>
            {/* Hero */}
            <div className="mb-7">
              <h2 className="text-section text-foreground mb-2">{t('track.title')}</h2>
              <p className="text-body-lg text-muted-foreground leading-relaxed">{t('track.subtitle')}</p>
            </div>

            {/* Track segmented control */}
            <div className="mb-7 p-1.5 rounded-2xl bg-muted/70 grid grid-cols-3 gap-1">
              {trackTabs.map(tab => {
                const Icon = tab.icon;
                ...
                return (
                  <button
                    ...
                    className={cn(
                      "flex flex-col items-center justify-center gap-1 h-16 rounded-xl transition-all duration-200 active:scale-[0.97]",
                      active
            ...
            {mainTabs.map(tab => {
              ...
              return (
                <button
                  ...
                  className={cn(
                    "flex flex-col items-center justify-center gap-1 py-2 rounded-2xl transition-all duration-200 active:scale-95",
