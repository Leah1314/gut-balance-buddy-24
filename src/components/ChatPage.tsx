...
const ChatPage = () => {
  ...
  useEffect(() => {
    const checkForUserData = async () => {
      try {
        ...
        const greetingMessage = hasAnyData ? "Hi! I'm your gut health coach. I can see you have some health data stored. I can help you understand your digestive patterns, suggest meal improvements, and answer questions about your symptoms. What would you like to know? {we_have_your_data}" : "Hi! I'm your gut health coach. I can help you understand your digestive patterns, suggest meal improvements, and answer questions about symptoms. You can add more information in the 'Track' or 'Health' tabs to get more personalized advice. What would you like to know?";
        setMessages([{
          id: '1',
  ...
  const quickPrompts = ["Analyze my recent meal patterns", "Help me understand my symptoms", "Suggest foods for better digestion", "What should I track daily?"];
  const sendMessage = async () => {
    ...
    try {
      ...
      const {
        data,
        error
      } = await supabase.functions.invoke('gut-health-chat', {
        body: {
  ...
  return <div className="flex flex-col h-full">
      ...
      <div className="text-center mb-6">
        <div className="flex items-center justify-center gap-3 mb-2">
          <MessageCircle className="w-8 h-8" style={{
          color: '#4A7C59'
        }} />
          ...
          {(hasUserData.health_info || hasUserData.track_history) && <div title="Your data is available">
              <Database className="w-5 h-5" style={{
            color: '#4A7C59'
          }} />
            </div>}
        </div>
      ...
      <div className="flex-1 overflow-y-auto space-y-4 mb-6 max-h-[calc(100vh-300px)]">
        ...
        {isLoading && <div className="flex justify-start">
            <div className="p-4 rounded-lg border flex items-center gap-2" style={{
      ...
      {messages.length === 1 && <div className="mb-6">
        ...
        color: '#2E2E2E'
          ...
          <div className="grid grid-cols-1 gap-2">
            {quickPrompts.map((prompt, index) => <Button key={index} variant="outline" size="sm" onClick={() => handleQuickPrompt(prompt)} className="text-left justify-start h-auto p-3 text-sm" style={{
          borderColor: '#D3D3D3',
          color: '#2E2E2E',
      ...
      borderColor: '#D3D3D3'
        ...
        <div className="flex gap-3">
          <Input value={inputMessage} onChange={e => setInputMessage(e.target.value)} onKeyPress={handleKeyPress} placeholder="Ask me about your gut health..." className="flex-1" style={{
          borderColor: '#D3D3D3',
          backgroundColor: '#FFFFFF',
          color: '#2E2E2E'
        }} disabled={isLoading} />
          <Button onClick={sendMessage} disabled={!inputMessage.trim() || isLoading} className="shrink-0 px-4" style={{
